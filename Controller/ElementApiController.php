...
class ElementApiController extends Controller
{
    ...
    public function renderAction(Request $Request, $context, $type, $name, $format = 'json'){
        ...
        if($element instanceof AbstractConfigurableElement){
            ...
            if($form->isValid()){
                ...
                return $this->createJsonResponse(json_encode(array('error' => $this->get('translator')->trans('invalid.element.parameter', null, 'kalamu') )));
            }
        }
    ...
    protected function getConfigForm(AbstractElement $element, $intention){
        ...
        if($intention == 'create'){
            $form->add('submit', 'submit', array('label' => $this->get('translator')->trans('element.add', null, 'kalamu'), 'attr' => array('class' => 'btn btn-success')));
        }elseif($intention == 'edit'){
            $form->add('submit', 'submit', array('label' => $this->get('translator')->trans('element.edit', null, 'kalamu'), 'attr' => array('class' => 'btn btn-success')));
        }
